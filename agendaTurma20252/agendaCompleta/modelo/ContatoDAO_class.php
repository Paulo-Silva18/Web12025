<?php
	...
	//operações básicas de banco de dados
		...
		public function __construct(){
			$conF = new ConnectionFactory(); //inicia a conexão
	...
	//alterar
		public function alterar($cont){
			try{
				...
				//Inicia a transação,consistência ao banco,sem erros
				$stmt->execute(); //execução do SQL	
				$this->con->commit(); 
				//rollback - voltar atrás, commit - confirmação de tudo ok
				/*$this->con->close();
				$this->con = null;*/	
			}
			catch(PDOException $ex){
		...
		public function excluir($cont){
			try{
				$num = $this->con->exec("DELETE FROM contato WHERE 
				id = " . $cont->getId());
		...
		public function exibir($id){			
			try{				
				$lista = $this->con->query("SELECT * FROM contato 
				WHERE id = " . $id);
